...
class DeepNetwork:
    ...
    def __init__(self, input_feats_shape):
        ...
        self.opt = None #blank for later compiling
...
#         self.loss_exp = None
    ...
    def accuracy(self, y_true, y_pred):
        ...
        '''
        index = tf.where(y_pred == y_true, 1, 0)
        acc = tf.reduce_sum(index) / len(y_pred)
        return acc
    ...
    def predict(self, x, output_layer_net_act=None):
        ...
        pred = tf.cast(tf.argmax(output_layer_net_act, axis=1), dtype = tf.int32)
        return pred
    ...
    def loss(self, out_net_act, y, eps=1e-16):
        ...
        if self.loss_name.lower() == 'cross_entropy':
            z_netact = arange_index(out_net_act, y)
            loss = -tf.reduce_mean(tf.math.log(z_netact + eps)) #Both sums the log and divides by the batch_size
            return loss
        
        elif self.loss_name.lower() == 'lp':
            y_onehot = tf.one_hot(y, depth = out_net_act.shape[1], on_value = 1.0, off_value = -1.0, dtype=tf.float32)
            loss = tf.reduce_sum(tf.reduce_sum(tf.pow(tf.abs(y_onehot  - out_net_act), self.loss_exp), axis=1)) / (out_net_act.shape[0] * out_net_act.shape[1])
            
            return loss


        #specified loss not supported
    ...
    def train_step(self, x_batch, y_batch):
        ...
        with tf.GradientTape() as tape:
            #Forward pass through entire network by using __Call__
            output_acts = self(x_batch)

            #Using loss function to compute loss
            loss = self.loss(output_acts, y_batch)
    ...
    def test_step(self, x_batch, y_batch):
        ...
        '''
        netact = self(x_batch)
        loss = self.loss(netact, y_batch)

        y_pred = self.predict(x_batch, netact)
    ...
    def fit(self, x, y, x_val=None, y_val=None, batch_size=128, max_epochs=10000, val_every=1, print_every=10,
        ...
        # this list will keep track of recent val losses for early stopping
        recent_val_losses = []
        lr_val_losses = []
        ...
        for e in range(max_epochs):
            ...
            for i in range(iterations):
                # first the minibatch with sampling w/ replacement
            ...
            train_loss_hist.append(tf.reduce_mean(loss))
            
            endTime = time.time()
            if e % val_every == 0:
                val_acc, val_loss = self.evaluate(x_val, y_val, batch_size)
                self.set_layer_training_mode(True)
                val_acc_hist.append(val_acc)
                val_loss_hist.append(val_loss)

                recent_val_losses, early_stop = self.early_stopping(recent_val_losses, val_loss, patience)
                lr_val_losses, lr_decay = self.early_stopping(lr_val_losses, val_loss, lr_patience)

                if verbose:
                    print(f'Currently on epoch {e}/{max_epochs-1}. Training loss: {loss[-1]}. Val loss: {val_loss}. Val acc: {val_acc}.')
            
            if verbose:
                print(f'Epoch {e} took: {endTime-startTime} secs')
    ...
    def early_stopping(self, recent_val_losses, curr_val_loss, patience):
        ...
        # First, we need to check if the recent list is too long
        if len(recent_val_losses) > patience + 1:
            recent_val_losses = recent_val_losses[1:]
        
        # Next, if the length of recent list is p + 1, then we can check for early stopping
        if len(recent_val_losses) == patience + 1:
            min_loss = min(recent_val_losses)
            if recent_val_losses[0] == min_loss:
                stop = True
    ...
    def lr_step_decay(self, lr_decay_rate):
        ...
        '''
        print("Learning rate before: ", self.opt.learning_rate.numpy())
        self.opt.learning_rate.assign(self.opt.learning_rate * lr_decay_rate)
        print("Learning rate after: ", self.opt.learning_rate.numpy())
