...
class Layer:
    ...
    '''
    def __init__(self, layer_name, activation, prev_layer_or_block, do_group_norm=False, tanh_beta = 1.0):
    ...
    def compute_net_activation(self, net_in):
        ...
        '''
        act_fun_name = self.get_act_fun_name()

        net_act = net_in
        if act_fun_name == 'relu':
            net_act = tf.keras.activations.relu(net_in)
        elif act_fun_name == 'linear':
            net_act = tf.keras.activations.linear(net_in)
        elif act_fun_name == 'softmax':
            net_act = tf.keras.activations.softmax(net_in)
        elif act_fun_name == 'tanh':
            net_act = tf.keras.activations.tanh(self.tanh_beta * net_in)
        else:
            raise Exception("Unsupported activation function: " + act_fun_name)
    ...
    def __call__(self, x):
        ...
        if self.is_doing_groupnorm() and self.gn_gain is not None:
...
class Dense(Layer):
    ...
    def init_params(self, input_shape):
        ...
        if self.wt_init == 'normal':
            ...
            stddev = tf.sqrt(k) / tf.sqrt(float(input_shape[-1]))

        self.wts = tf.Variable(tf.random.normal([input_shape[-1], self.get_num_units()], stddev=stddev), name='wts')
        self.b = tf.Variable(tf.zeros(self.get_num_units()), name='bias')
    ...
    def compute_group_norm(self, net_in, eps=0.001):
        ...
        if self.num_groups is None:
            self.set_num_groups(self.get_num_units() // 8)

        if self.num_groups > 8:
            self.set_num_groups(8)
        
        # group the net in
        net_in_grouped = tf.reshape(net_in, [B, self.num_groups, H // self.num_groups])
        
        # normalize within the G groups
        mean = tf.reduce_mean(net_in_grouped, axis = 2, keepdims=True)
        stdev = tf.math.reduce_std(net_in_grouped, axis = 2, keepdims=True)
        net_in2 = (net_in_grouped - mean) / (stdev + eps)

        # dissolve groups
        net_in3 = tf.reshape(net_in2, [B, H])

        # scale & shift
        net_in_gn = self.gn_gain * net_in3 + self.gn_bias
...
class Dropout(Layer):
    ...
    def compute_net_input(self, x):
        ...
        if self.get_mode():
            r = 1 - self.rate
            mask = tf.random.uniform(shape = list(x.shape))
            mask = tf.cast(tf.where(mask > r, 0, 1), dtype = "float32")
            net_in = (x * mask) * (1 / r)
        else:
            net_in = x
        return net_in
...
class Flatten(Layer):
    ...
    def compute_net_input(self, x):
        ...
        '''
        net_in = tf.reshape(x, [x.shape[0], -1])
